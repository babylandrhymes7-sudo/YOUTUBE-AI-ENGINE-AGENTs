...
def isolated_notification_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep duplicate ledgers and queues isolated between tests."""

    state_dir = tmp_path / "notification_state"
    state_dir.mkdir()
    monkeypatch.setattr("notifications.manager.settings.notification_state_dir", state_dir)
    monkeypatch.setattr("notifications.engine.settings.notification_state_dir", state_dir)
