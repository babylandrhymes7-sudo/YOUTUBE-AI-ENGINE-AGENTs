...
from app.daily_flow import run_daily_report_from_local_json
...
class JobManager:
    ...
    def configure(self) -> None:
        ...
        self.register_job(self._news_updates_job())
    ...
    def _news_updates_job(self) -> JobDefinition:
        """Build the recurring news-only collection and alert job."""

        return JobDefinition(
            name="news_updates",
            func=self._news_updates_task,
            trigger=IntervalTrigger(minutes=settings.scheduler_every_30_minutes, timezone=settings.scheduler_timezone),
        )
    ...
    def _daily_report_task(self) -> None:
        """Run the local daily report flow from structured JSON handoff data."""

        logger.info("Daily report job triggered at {}", datetime.utcnow().isoformat())
        session_factory = get_session_factory()
        with session_factory() as session:
            result = run_daily_report_from_local_json(session, report_type="daily")
        logger.info("Daily report generated pdf={} duration={:.3f}s", result.pdf_path, result.generation_seconds)

    def _weekly_report_task(self) -> None:
        """Run the local weekly YouTube channel report flow."""

        logger.info("Weekly report job triggered at {}", datetime.utcnow().isoformat())
        session_factory = get_session_factory()
        with session_factory() as session:
            result = run_daily_report_from_local_json(session, report_type="weekly")
        logger.info("Weekly report generated pdf={} duration={:.3f}s", result.pdf_path, result.generation_seconds)

    def _monthly_report_task(self) -> None:
        """Run the local monthly YouTube channel report flow."""

        logger.info("Monthly report job triggered at {}", datetime.utcnow().isoformat())
        session_factory = get_session_factory()
        with session_factory() as session:
            result = run_daily_report_from_local_json(session, report_type="monthly")
        logger.info("Monthly report generated pdf={} duration={:.3f}s", result.pdf_path, result.generation_seconds)

    def _news_updates_task(self) -> None:
        """Run the web/news collection -> AI analysis -> notification flow."""

        logger.info("News updates job triggered at {}", datetime.utcnow().isoformat())
        session_factory = get_session_factory()
        with session_factory() as session:
            result = asyncio.run(run_news_alert_flow(session))
        logger.info(
            "News updates completed articles={} duration={:.3f}s",
            result.collection_stats.get("articles"),
